...
struct RFID_DCONTAINER
{

    uint8_t storedCard_DataCont[4];
    uint8_t readCard_DataCont[4];
    uint8_t masterCard_DataCont[4];

    uint8_t masterCard_FirstName[18];
    uint8_t masterCard_LastName[18];

    uint8_t readCard_FirstName[18];
    uint8_t readCard_LastName[18];
};

RFID_DCONTAINER RFID_DataHolder;
...
void setup()
{
    ...
    // ! Checks if we have Master RFID Register in EEPROM of our Arduino.
    if (!isMasterRFIDExist())
    {
        Serial.println("There are no existing Master Card in EEPROM of Arduino. Please put a card to be a master.");
        do
        {
            RFID_ScanCard();
        } while (!RFID_DataHolder.readCard_DataCont);
    }
    else
    {
        assignVar_ofMasterRFID();
        Serial.println("Master Card Detected in EEPROM of Arduino!");
        dispInfo_MasterRFID();
    }
}
...
inline void assignVar_ofMasterRFID()
{
    //! NEEDS A VARIABLE FOR THIS FUNCTION
    for (uint8_t MasterUIDByte = 0; MasterUIDByte < 4; MasterUIDByte++)
    {
        RFID_DataHolder.masterCard_DataCont[MasterUIDByte] = EEPROM.read(1 + MasterUIDByte);
    }
    return;
}

inline bool dispInfo_MasterRFID()
{

}

inline uint8_t count_RFIDRecords()
{
    // ! Do something about it. MAXIMUM OF CARDS IS 3.
    //return EEPROM.read();
}
...
bool RFID_ScanCard()
{
    if (MFRC522.PICC_IsNewCardPresent() || MFRC522.PICC_ReadCardSerial())
    {
        ...
        {
            RFID_DataHolder.readCard_DataCont[ReadUIDByte] = MFRC522.uid.uidByte[ReadUIDByte];
            RFID_DataHolder.PICC_DumpDetailsToSerial(&(mfrc522.uid));
        }
...
void OtherRFID_Add()
{
    // ! Add function of limiting cards to 3. So therefore, 2 other cards and one master card.
}

void KnownRFID_Remove()
{
}
    ...
    for (k = 0; k < 16; k++)
    {
        for (c = 0; c < 32; c++)
        {
            ...
            if (y < 0)
                y = 0;
            //  adjust size
            if (x + w > TFTScreen.width())
                w = TFTScreen.width() - x;
            if (y + h > TFTScreen.height())
                h = TFTScreen.height() - y;
            if (fill)
            {
...
void randomCircles(bool fill)
{
    ...
    for (k = 0; k < TFTScreen.height(); k++)
    {
        for (c = 0; c < TFTScreen.height() / 2; c++)
        {
            ...
            if (x - r < 0)
                r = x;
            if (x + r > (TFTScreen.width() - 1))
                r = (TFTScreen.width() - 1) - x;
            if (y - r < 0)
                r = y;
            if (y + r > (TFTScreen.height() - 1))
                r = (TFTScreen.height() - 1) - y;
            if (fill)
            {
